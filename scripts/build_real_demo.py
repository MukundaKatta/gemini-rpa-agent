"""Build the final gemini-rpa-agent demo video.
...
Total ~60s, well under typical 3-minute hackathon submission limits.
Centerpiece is real footage of the deployed Streamlit dashboard
answering an RPA-failure-diagnosis question by walking the RPA MCP
tools (list_workflows + get_workflow_run + get_step_output +
suggest_retry) and citing the verbatim channel_not_found error payload
from the failing slack-notify step.
"""
...
def draw_intro(img, d):
    ...
    d.text((48, H - 44),
           "github.com/MukundaKatta/gemini-rpa-agent",
           font=font(22), fill=FG_MUTED)
    d.text((W - 270, H - 44), "Apache 2.0", font=font(22), fill=FG_MUTED)
    d.text((96, 220), "gemini-rpa-agent", font=font(108), fill=FG)
    d.rectangle([(96, 360), (340, 372)], fill=ACCENT)
    d.text((96, 410),
           "Plain-English RPA questions, walked end-to-end through",
           font=font(40), fill=FG_MUTED)
    d.text((96, 470),
           "an RPA MCP, with verbatim error payloads cited byte-for-byte.",
           font=font(40), fill=FG_MUTED)
    d.text((96, 600),
           "UiPath AgentHack 2026",
           font=font(32), fill=FG)
    d.text((96, 645),
           "$50K prize pool, deadline 2026-06-29.",
           font=font(32), fill=FG_MUTED)
    d.text((96, 760),
           "What follows is real footage of the deployed",
           font=font(28, italic=True), fill=FG_MUTED)
    d.text((96, 800),
           "Cloud Run dashboard diagnosing a live RPA failure.",
           font=font(28, italic=True), fill=FG_MUTED)


def draw_outro(img, d):
    d.rectangle([(0, 0), (W, H)], fill=BG)
    d.text((96, 180), "gemini-rpa-agent", font=font(72), fill=FG)
    d.rectangle([(96, 280), (340, 292)], fill=ACCENT)
    d.text((96, 330),
           "github.com/MukundaKatta/gemini-rpa-agent",
           font=font(30, mono=True), fill=ACCENT_2)
    d.text((96, 410),
           "gemini-rpa-agent-1029931682737.us-central1.run.app",
    ...
    d.text((96, 590),
           ...
           font=font(32), fill=FG_MUTED)
    d.text((96, 640),
           "+ RPA MCP (list_workflows, get_step_output, suggest_retry, ...)",
           font=font(32), fill=FG_MUTED)
    d.text((96, 760),
           "The error payload in this demo is copied byte-for-byte",
           font=font(26, italic=True), fill=FG_MUTED)
    d.text((96, 800),
           "from the failing step. No paraphrasing.",
           font=font(26, italic=True), fill=FG_MUTED)
    d.text((96, 870),
...
INTRO_NARRATION = (
    "Gemini R P A agent. A workflow-automation triage agent on Google "
    "Cloud Agent Builder, wired to an R P A M C P server. Submission "
    "for UiPath Agent Hack two thousand twenty six. What follows is real "
    "footage of the deployed Cloud Run dashboard walking the R P A tools "
    "and citing the verbatim error payload from the failing step."
)


OUTRO_NARRATION = (
    "Every line of evidence you just saw was copied byte for byte from "
    "the failing step's error payload. The agent never paraphrases. Built "
    "on the A D K with Gemini two point five Flash, wired to an R P A "
    "M C P server, list workflows, get workflow run, get step output, "
    "suggest retry. Apache two point zero. Thank you."
)
...
def main():
    outdir = Path("/Users/ubl/gemini-rpa-agent/.video-build")
